...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.down1 = nn.Sequential(*list(self.features.children())[0:2])
        self.down2 = nn.Sequential(*list(self.features.children())[2:4])
        self.down3 = nn.Sequential(*list(self.features.children())[4:6])
        self.down4 = nn.Sequential(*list(self.features.children())[6:8])

        self.up1 = BasicConv(256, 128, kernel_size=4, relu=True, stride=2, transpose=True)
        self.up2 = BasicConv(128, 64, kernel_size=4, relu=True, stride=2, transpose=True)
        self.up3 = BasicConv(64, 64, kernel_size=4, relu=True, stride=2, transpose=True)
        self.up4 = BasicConv(64, 3, kernel_size=4, relu=True, stride=2, transpose=True)
    ...
    def forward(self, x):
      ...
      #   print(x.shape)
        ...
        x4 = self.down4(x3) 
                                                ...
                                                #        x2 torch.Size([2, 64, 56, 56])
                                                #        x3 torch.Size([2, 128, 28, 28])
                                                #        x4 torch.Size([2, 256, 14, 14])
